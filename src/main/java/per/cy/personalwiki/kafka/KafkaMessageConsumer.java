...
public class KafkaMessageConsumer {
    ...
    public void consumeVote(ConsumerRecord<String, String> record) {
    ...
    public void consumeDeleteCache(ConsumerRecord<String, String> record) {
        String cacheKey = record.value();
        logger.info("Kafka收到删除消息，对应缓存的key为"+cacheKey+",将消息放入延时队列中");
        delayQueue.offer(new DelayedMessage(cacheKey,1, TimeUnit.SECONDS,0));
    }
